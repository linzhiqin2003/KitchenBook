...
| 模型 | 说明 | 关键字段 |
|------|------|----------|
| `Ingredient` | 食材库存 | `name`, `category`, `quantity`, `unit`, `threshold` |
...
**问题**：原库存管理页面过于简陋，所有食材平铺展示，缺少分类管理。

**解决方案**：

1. **后端：Ingredient 模型添加 category 字段**
   - 分类选项：肉类(meat)、海鲜(seafood)、蔬菜(vegetable)、调味料(seasoning)、主食/干货(staple)、乳制品(dairy)、其他(other)
   - Serializer 新增 `category` 和 `category_display` 字段
   - 迁移文件：`0006_add_ingredient_category.py`

2. **前端 InventoryView 全面重构**
   - 🎨 **视觉升级**：渐变背景、毛玻璃效果、统计卡片
   - 📊 **统计面板**：显示食材总数、有库存、缺货数量
   - 🏷️ **分类筛选**：顶部标签栏快速筛选分类
   - 📦 **分类分组**：每个分类独立展示区域，带渐变标题栏和图标
   - ✏️ **编辑功能**：支持修改食材名称和分类
   - 🚨 **缺货标识**：缺货食材红色高亮 + 角标提示
...
**需求**：将技术博客从"LZQ的私人厨房"中独立出来，作为独立的个人技术博客页面。

**解决方案**：

1. **App.vue 布局判断**
   - 新增 `isBlogPage` 计算属性，判断 `/blog` 和 `/blog/:slug` 路由
   - 博客页面使用独立的全屏布局，不显示厨房导航栏和页脚

2. **BlogListView.vue 全面重构**
   - 🎨 **独立品牌**：顶部导航显示 "LZQ's Tech Blog" + "技术沉淀 · 持续进化"
   - 🌙 **暗色主题**：深色背景 (#0a0a0f)，紫色/品红色渐变光晕
   - 🔗 **独立导航**：私人厨房、AI 实验室、GitHub 链接
   - 📊 **统计卡片**：文章数、阅读次数、标签数（带渐变发光效果）
   - 🔍 **搜索筛选**：暗色主题的搜索框和标签筛选
   - 📰 **文章卡片**：暗色卡片设计，悬停边框高亮
   - 📜 **独立页脚**：关于、快速链接、联系方式、版权信息

3. **BlogPostView.vue 全面重构**
   - 🔝 **独立导航栏**：与博客列表页一致
   - 🖼️ **暗色文章头部**：紫色渐变背景 + 网格装饰
   - 📖 **暗色正文样式**：代码块、引用块、链接等适配暗色主题
   - 🔙 **返回按钮**：带动画的 "< 返回博客列表"
   - 📜 **独立页脚**：与博客列表页一致

**技术要点**：
- 博客页面完全独立于厨房项目，有自己的视觉体系
- 深色主题 + 紫色/品红色渐变，营造科技感
- 保留与厨房项目、AI 实验室的互相导航
