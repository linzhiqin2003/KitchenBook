    ...
    <div class="relative z-10 w-full max-w-6xl flex flex-col items-center">
        ...
        <div class="w-full min-h-[600px] flex flex-col items-center justify-start pt-4">
            ...
            <transition name="smooth" mode="out-in">
                ...
                <!-- Step 2: Shuffle Animation - Enhanced -->
                <div v-else-if="step === 'shuffle'" key="shuffle" class="w-full h-80 flex flex-col items-center justify-center relative">
                    <div class="relative w-40 h-60 sm:w-48 sm:h-72 shuffle-container">
                        <!-- Multiple card layers with staggered animation -->
                        <div 
                            v-for="(card, index) in shuffledDeck.slice(0, 12)" 
                            :key="card.id"
                            class="absolute w-full h-full rounded-xl shadow-2xl shuffle-card"
                            :style="{
                                '--delay': `${index * 0.08}s`,
                                '--offset': `${(index - 6) * 3}px`,
                                zIndex: 12 - index
                        ...
                        >
                            <div class="w-full h-full bg-gradient-to-br from-indigo-950 via-purple-900 to-black border border-mystic-gold rounded-xl">
                                <div class="absolute inset-0 flex items-center justify-center text-2xl sm:text-3xl text-mystic-gold opacity-40">✧</div>
                            </div>
                        </div>
                    </div>
                    <div class="mt-8 text-mystic-purple tracking-widest uppercase text-sm flex items-center gap-2">
                        ...
                        <span class="ml-2">Shuffling the Arcana</span>
                    </div>
                </div>
...
@keyframes shuffleCard {
    0%, 100% {
        transform: translateX(var(--offset, 0)) translateY(0) rotateY(0deg) rotateZ(0deg);
    }
    15% {
        transform: translateX(calc(var(--offset, 0) + 40px)) translateY(-20px) rotateY(15deg) rotateZ(5deg);
    }
    30% {
        transform: translateX(calc(var(--offset, 0) - 30px)) translateY(-40px) rotateY(-10deg) rotateZ(-3deg);
    }
    50% {
        transform: translateX(var(--offset, 0)) translateY(-60px) rotateY(0deg) rotateZ(0deg) scale(1.05);
    }
    70% {
        transform: translateX(calc(var(--offset, 0) + 20px)) translateY(-30px) rotateY(8deg) rotateZ(2deg);
    }
    85% {
        transform: translateX(calc(var(--offset, 0) - 15px)) translateY(-10px) rotateY(-5deg) rotateZ(-1deg);
    }
}
