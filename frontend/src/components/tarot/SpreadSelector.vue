<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl w-full px-4">
    ...
    >
      <!-- Card Count Badge -->
      <div class="absolute -top-3 -right-3 bg-mystic-gold text-black text-sm font-bold px-3 py-1.5 rounded-full shadow-lg z-20">
        {{ spread.card_count }}牌
      </div>
      
      <!-- Spread Info - Add relative z-10 to ensure visibility -->
      <div class="relative z-10">
        <h3 class="text-2xl text-mystic-gold mb-3">{{ spread.name_cn }}</h3>
        <p class="text-base text-gray-400 mb-5 line-clamp-2">{{ spread.description }}</p>
