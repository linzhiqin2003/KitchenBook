            ...
            for round_num in range(max_rounds):
                ...
                if finish_reason == "tool_calls" and choice.message.tool_calls:
                    ...
                    for tc in choice.message.tool_calls:
                        ...
                        tool_content = json.dumps({"error": error_str}, ensure_ascii=False) if error_str else (result_str or "")
                        messages.append({
                            "role": "tool",
                            "tool_call_id": tc.id,
...
@mcp.tool()
async def ailab_ocr(image_url: str) -> str:
    """
            ...
            payload = {
                ...
                "messages": [{
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": "请将图片内容转换为 Markdown 格式。保留公式、表格等结构。只输出内容，不要添加额外说明。"},
                    ],
                }],
                "max_tokens": 4096,
                "temperature": 0.1,
...
@mcp.tool()
async def ailab_transcribe(audio_url: str, language: str = "zh") -> str:
    """
    语音转录——将音频文件转换为文字。
...
@mcp.tool()
async def ailab_emoji_generate(template_id: str, image_url: str) -> str:
    """
    表情包视频生成——上传人像图片，套用表情模板生成动态表情包。

    Args:
        template_id: 表情模板 ID（如 "mua"、"wink"）
    ...
    def _run_emoji():
        ...
        try:
            ...
            return f"错误：EmojiService 初始化失败 — {e}"
            ...
            try:
                ...
                for _ in range(30):
                    ...
                    if status.get("status") == "completed":
                        return f"表情包生成成功！视频 URL: {status.get('video_url', '')}"
    ...
    return await asyncio.to_thread(_run_emoji)
