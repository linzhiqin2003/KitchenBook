...
### 4. 修改 hermes 配置 —— MCP HTTP transport + 容器 bind 0.0.0.0

#### 4.a `~/.hermes/.env` —— `API_SERVER_HOST` 改成 `0.0.0.0`

容器化模式下 hermes 必须 bind `0.0.0.0:8642` 才能让 docker port forward 转发；
否则容器内 healthcheck 通过但 host 上 `curl 127.0.0.1:8642` 会 connection reset。

```bash
cp ~/.hermes/.env ~/.hermes/.env.bak.$(date +%s)
sed -i 's|^API_SERVER_HOST=.*|API_SERVER_HOST=0.0.0.0|' ~/.hermes/.env
grep API_SERVER_HOST ~/.hermes/.env  # 确认 0.0.0.0
```

> ⚠️ **回滚到 host 模式前必须改回 `127.0.0.1`** —— 否则 host 上的 hermes 会
> bind 0.0.0.0:8642，公网安全组没拦的话会暴露。

#### 4.b `~/.hermes/config.yaml` —— `mcp_servers.ailab` 改 HTTP transport
...
容器跑挂了想立刻回原方案，**4 步、零数据丢失**（`~/.hermes/` 是 bind mount 不动）：

```bash
# 1. 停容器 + 停 host MCP
sudo systemctl stop hermes-docker.service ailab-mcp.service
sudo systemctl disable hermes-docker.service ailab-mcp.service

# 2. 还原 hermes config (mcp_servers.ailab stdio + skill_paths host 路径)
cp ~/.hermes/config.yaml.bak.<timestamp> ~/.hermes/config.yaml

# 3. ⚠️ 改回 ~/.hermes/.env: API_SERVER_HOST=127.0.0.1
#    否则 host 模式下 hermes 会 bind 0.0.0.0:8642，公网暴露
sed -i 's|^API_SERVER_HOST=.*|API_SERVER_HOST=127.0.0.1|' ~/.hermes/.env
